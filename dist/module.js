...
var SWIM_SPEED_ADVANCEMENT_ID = "bfeSwim000000000";
...
function formatUnlabeledBulletList(trait, { includeHeading = true } = {}) {
  ...
  const list = bullets.map((item2) => `<li>${escapeHTML(item2)}</li>`).join("\n");
  return [
...
function parseChoiceListTrait(trait) {
  ...
  for (const rawLine of trait.lines ?? []) {
    const line = String(rawLine || "").trim();
    const match = line.match(/^[•\-*]\s*([A-Z][A-Za-z'’]*(?:\s+[A-Z][A-Za-z'’]*){0,2})\.\s*(.*)$/);
    if (match) {
